...
<body>
	...
	<form:form id="searchForm" modelAttribute="cdFootballFollowOrder" action="${ctx}/cfootballorder/cdFootballFollowOrder/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="cdFootballFollowOrder">
			<tr>
				<td><a href="${ctx}/cfootballorder/cdFootballFollowOrder/form?id=${cdFootballFollowOrder.id}">${cdFootballFollowOrder.orderNum}</a></td>
				<td><c:choose>
				...
				<td><%--${cdFootballFollowOrder.stauts}--%>
					<c:choose>
						<c:when test="${cdFootballFollowOrder.stauts==1}">
							未付款
						</c:when>
						<c:when test="${cdFootballFollowOrder.stauts==2}">
							已付款
						</c:when>
						<c:when test="${cdFootballFollowOrder.stauts==3}">
							已出票
						</c:when>
						<c:otherwise>
							订单异常
				...
				<shiro:hasPermission name="cfootballorder:cdFootballFollowOrder:edit">
					<td>
						...
						<a href="${ctx}/cfootballorder/cdFootballFollowOrder/delete?id=${cdFootballFollowOrder.id}" onclick="return confirmx('确认要删除该竞彩足球订单吗？', this.href)">删除</a>
					</td>
				</shiro:hasPermission>
			</tr>
		</c:forEach>
