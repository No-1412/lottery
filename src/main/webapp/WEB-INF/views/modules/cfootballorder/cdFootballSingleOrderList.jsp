...
<body>
	...
	<form:form id="searchForm" modelAttribute="cdFootballSingleOrder" action="${ctx}/cfootballorder/cdFootballSingleOrder/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead>
			<tr>
				...
				<th>下单时间</th>
				<th>订单状态</th>
				<shiro:hasPermission name="cfootballorder:cdFootballSingleOrder:edit"><th>操作</th></shiro:hasPermission>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${page.list}" var="cdFootballSingleOrder">
			<tr>
				<td><a href="${ctx}/cfootballorder/cdFootballSingleOrder/form?id=${cdFootballSingleOrder.id}">${cdFootballSingleOrder.orderNum}</a></td>
			...
			<%--	<td>${cdFootballSingleOrder.uid}</td>--%>
				...
				<shiro:hasPermission name="cfootballorder:cdFootballSingleOrder:edit">
					<td>
						...
						<a href="${ctx}/cfootballorder/cdFootballSingleOrder/delete?id=${cdFootballSingleOrder.id}" onclick="return confirmx('确认要删除该竞彩足球订单吗？', this.href)">删除</a>
					</td>
				</shiro:hasPermission>
			</tr>
		</c:forEach>
