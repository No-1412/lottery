  ...
  createPayment: function(charge_json, callback, signature, debug) {
    ...
    if(typeof charge_json == "string"){
      try{
        ...
        alert("112222222json_decode_fail");
        return;
      }
    }else{
      charge = charge_json;
    ...
    if (!hasOwn.call(charge['credential'], channel)) {
      ...
      alert("invalid_charge   no_valid_channel_credential");
      return;
    }
    ...
    if (channel == channels.upmp_wap) {  // 调起银联支付控件，客户端�?要安装银联支付控件才能调�?
    ...
    } else if (channel == channels.alipay_wap) {  // 调起支付宝手机网页支�?
